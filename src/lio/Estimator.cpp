...
Estimator::Estimator(const float& filter_corner, const float& filter_surf){
    ...
    for(auto& c : laserCloudCornerLast)
        c.reset(new pcl::PointCloud<PointType>);

    laserCloudSurfLast.resize(SLIDEWINDOWSIZE);
    for(auto& c : laserCloudSurfLast)
        c.reset(new pcl::PointCloud<PointType>);

    laserCloudNonFeatureLast.resize(SLIDEWINDOWSIZE);
    for(auto& c : laserCloudNonFeatureLast)
        c.reset(new pcl::PointCloud<PointType>);

    laserCloudCornerStack.resize(SLIDEWINDOWSIZE);
    for(auto& c : laserCloudCornerStack)
        c.reset(new pcl::PointCloud<PointType>);
    ...
    threadMap_ = std::thread(&Estimator::threadMapIncrement, this);
}
...
Estimator::threadMapIncrement() {
	auto cornInMapCoordAccumulated	= makeShared<pcl::PointCloud<PointType>>();
	auto surfInMapCoordAccumulated	= makeShared<pcl::PointCloud<PointType>>();
	auto NonfInMapCoordAccumulated	= makeShared<pcl::PointCloud<PointType>>();
	...
	while(true) {
        {
            ...
            if(!laserCloudCornerForMap->empty()) {
                ...
                auto cornInMapCoord			= makeShared<pcl::PointCloud<PointType>>();
                auto surfInMapCoord			= makeShared<pcl::PointCloud<PointType>>();
                auto NonfInMapCoord			= makeShared<pcl::PointCloud<PointType>>();
                ...
                *cornInMapCoordAccumulated += *cornInMapCoord;
                *surfInMapCoordAccumulated += *surfInMapCoord;
                *NonfInMapCoordAccumulated += *NonfInMapCoord;

                // cornInMapCoord->clear();
                // surfInMapCoord->clear();
                // NonfInMapCoord->clear();

                if(map_update_ID % map_skip_frame == 0) {
                    map_manager->MapIncrement(cornInMapCoordAccumulated, 
                                            surfInMapCoordAccumulated, 
                                            NonfInMapCoordAccumulated,
                                            transform);

                    cornInMapCoordAccumulated->clear();
                    surfInMapCoordAccumulated->clear();
                    NonfInMapCoordAccumulated->clear();
                }
            }
            // else
            //     locker.unlock();
...
void Estimator::processPointToLine(std::vector<ceres::CostFunction *>& edges,
    ...
    for (int i = 0; i < laserCloudCorner->size(); i++) {
        ...
        const auto& p = laserCloudCorner->points[i];
        if(std::isnan(p.x) || std::isnan(p.y) ||std::isnan(p.z)) continue;
        _pointSel.getVector3fMap() = guess * laserCloudCorner->points[i].getVector3fMap();
        ...
        if(GlobalCornerMap[id].points.size() > 100) {
            ...
            if (_pointSearchSqDis[4] < thres_dist) {
                ...
                // _matA1(2, 1) = a23;
                // _matA1(2, 2) = a33;
                EIGEN_ALIGN16 Eigen::Matrix<float, 3, 3> covariance;
                Eigen::Matrix<float, 4, 1> centroid
                pcl::computeMeanAndCovarianceMatrix(GlobalCornerMap[id], _pointSearchInd, covariance, centroid)
        ...
        if(laserCloudCornerLocal->points.size() > 20 ) {
            ...
            if (_pointSearchSqDis2[4] < thres_dist) {
                ...
                EIGEN_ALIGN16 Eigen::Matrix<float, 3, 3> covariance;
                Eigen::Matrix<float, 4, 1> centroid
                pcl::computeMeanAndCovarianceMatrix(GlobalCornerMap[id], _pointSearchInd2, covariance, centroid)
...
void Estimator::processPointToPlan(std::vector<ceres::CostFunction *>& edges,
                                   ...
                                   const pcl::KdTreeFLANN<PointType>::Ptr& kdtreeLocal,
                                   const Eigen::Affine3f& exTlb,
                                //    const Eigen::Affine3f& m4d) {
                                   const Eigen::Affine3f& guess) {
    // Eigen::Matrix4d Tbl = Eigen::Matrix4d::Identity();
    // Tbl.topLeftCorner(3,3) = exTlb.topLeftCorner(3,3).transpose();
    // Tbl.topRightCorner(3,1) = -1.0 * Tbl.topLeftCorner(3,3) * exTlb.topRightCorner(3,1);
    Eigen::Affine3f Tbl = exTlb.inverse();
    ...
    for (int i = 0; i < laserCloudSurf->size(); i++) {
        const auto& p = laserCloudSurf->points[i];
        if(std::isnan(p.x) || std::isnan(p.y) ||std::isnan(p.z)) continue;
        // _pointOri = laserCloudSurf->points[i];
        // pointAssociateToMap(&_pointOri, &_pointSel, m4d);
        _pointSel.getVector3fMap() = guess * p.getVector3fMap();
        ...
        if(GlobalSurfMap[id].points.size() > 50) {
            ...
            if (_pointSearchSqDis[4] < 1.0) {
                ...
                for (int j = 0; j < 5; j++) {//point plane distance
        ...
        //when surf->map association failed,find surf->local association
...
void Estimator::processNonFeatureICP(std::vector<ceres::CostFunction *>& edges,
    ...
    for (int i = 0; i < laserCloudNonFeature->size(); i++) {
        const auto& p = laserCloudNonFeature->points[i];
        if(std::isnan(p.x) || std::isnan(p.y) ||std::isnan(p.z)) continue;
...
void Estimator::Estimate(std::list<LidarFrame>& lidarFrameList,
  ...
  kdtreeNonFeatureFromLocal->setInputCloud(laserCloudNonFeatureFromLocal);
    ...
    if(lidarFrameList.size() == SLIDEWINDOWSIZE) {
    ...
    for(int iterOpt = 0; iterOpt < max_iters; ++iterOpt) {
        ...
        if(lidarFrameList.size() == SLIDEWINDOWSIZE) {
            thres_dist = 1.0;
            if(iterOpt == 0) {
                for(int f = 0; f < lidarFrameList.size(); ++f) {
                    int cntFtu = 0;
        ...
        } else {//lidarFrameList.size() != SLIDEWINDOWSIZE
            if(iterOpt == 0) {
                thres_dist = 10.0;
            } else {
                thres_dist = 1.0;
        ...
        double deltaR = (q_before_opti.angularDistance(q_after_opti)) * RAD_TO_DEG;//姿态a旋转到姿态b所需绕某个轴的最短角度
 ...
 * saved in localMap
 */
void Estimator<PointType>::MapIncrementLocal(const pcl::PointCloud<PointType>::Ptr& cornerStack,
                                            const pcl::PointCloud<PointType>::Ptr& surfStack,
                                            const pcl::PointCloud<PointType>::Ptr& nonFeatureStack,
                                            const Eigen::Matrix4f& transformTobeMapped){
    size_t Id = localMapID % localMapWindowSize;
    // localCornerMap[Id]->clear();
    // localSurfMap[Id]->clear();
    // localNonFeatureMap[Id]->clear();
    // for (int i = 0; i < laserCloudCornerStackNum; i++) {
    //     pointAssociateToMap(&cornerStack->points[i], &pointSel, transformTobeMapped);
    //     localCornerMap[Id]->push_back(pointSel);
    // }
    pcl::transformPointCloud(cornerStack, localCornerMap[Id], transformTobeMapped)

    // for (int i = 0; i < laserCloudSurfStackNum; i++) {
    //     pointAssociateToMap(&surfStack->points[i], &pointSel2, transformTobeMapped);
    //     localSurfMap[Id]->push_back(pointSel2);
    // }
    pcl::transformPointCloud(surfStack, localSurfMap[Id], transformTobeMapped)

    // for (int i = 0; i < laserCloudNonFeatureStackNum; i++) {
    //     pointAssociateToMap(&nonFeatureStack->points[i], &pointSel2, transformTobeMapped);
    //     localNonFeatureMap[Id]->push_back(pointSel2);
    // }
    pcl::transformPointCloud(nonFeatureStack, localNonFeatureMap[Id], transformTobeMapped)
    ...
    auto temp = makeShared<pcl::PointCloud<PointType>>();
