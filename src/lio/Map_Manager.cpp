...
int g_gridNumX = 21;// 子cube沿宽、高、深度分割个数
int g_gridNumY = 11;
int g_gridNumZ = 21;
int g_gridNum = g_gridNumX * g_gridNumY * g_gridNumZ;//cube的总数量:4851
...
void MAP_MANAGER<PointType>::MapIncrement(const pcl::PointCloud<PointType>::Ptr& laserCloudCornerStack,
...
//   for (int i = 0; i < laserCloudCornerStack->size(); i++) {
    ...
    // }
    for (const auto& p : *laserCloudCornerStack) {
        size_t cubeInd;
        if(pntToCubeIdx(p, cubeInd)) {
    ...
    //     }
    // }
    for (const auto& p : *laserCloudSurfStack) {
        size_t cubeInd;
        if(pntToCubeIdx(p, cubeInd)) {
    ...
    //     }
    // }
    for (const auto& p : *laserCloudNonFeatureStack) {
        size_t cubeInd;
        if(pntToCubeIdx(p, cubeInd)) {
            laserCloudNonFeatureArray[cubeInd]->push_back(p);
            NonFeatureChangeFlag[cubeInd] = true;
        }
    }
    ...
    auto tmp = makeShared<pcl::PointCloud<PointType>>();
    for(int i = 0; i < laserCloudNum; i++) {
        if(CornerChangeFlag[i]) {
            if(laserCloudCornerArray[i]->points.size() > 300){
                ...
                // auto tmp = laserCloudCornerArrayStack[i];
        ...
        if(SurfChangeFlag[i]){
            if(laserCloudSurfArray[i]->points.size() > 300){
                ...
                // auto tmp = laserCloudSurfArrayStack[i];
        ...
        if(NonFeatureChangeFlag[i]){
            if(laserCloudNonFeatureArray[i]->points.size() > 300){
                ...
                // auto tmp = laserCloudNonFeatureArrayStack[i];
                // laserCloudNonFeatureArrayStack[i] = laserCloudNonFeatureArray[i];
                downSizeFilterNonFeature.filter(*tmp);
 ...
 */
void MAP_MANAGER::MapMove(const Eigen::Matrix4d& map_T_lidar) {
    const Eigen::Vector3f map_t_lidar = map_T_lidar.topRightCorner(3, 1);

    // PointType pointOnYAxis;
    // pointOnYAxis.x = 0.0;
    // pointOnYAxis.y = 0.0;
    // pointOnYAxis.z = 10.0;
    // pointAssociateToMap(pointOnYAxis, pointOnYAxis, map_T_lidar);

    // int centerCubeI = int((map_t_lidar.x() + 25.0) / 50.0) + laserCloudCenDepth;
    // int centerCubeJ = int((map_t_lidar.y() + 25.0) / 50.0) + laserCloudCenWidth;
    // int centerCubeK = int((map_t_lidar.z() + 25.0) / 50.0) + laserCloudCenHeight;

    // if (map_t_lidar.x() + 25.0 < 0) centerCubeI--;
    // if (map_t_lidar.y() + 25.0 < 0) centerCubeJ--;
    // if (map_t_lidar.z() + 25.0 < 0) centerCubeK--;

    int centerCubeI;
    int centerCubeJ;
    int centerCubeK;
    pntToCubeIdx(map_t_lidar, 
                laserCloudCenDepth, laserCloudCenWidth, laserCloudCenHeight, 
    ...
    while (centerCubeI < 8) {
        for (int j = 0; j < laserCloudWidth; j++) {
            for (int k = 0; k < laserCloudHeight; k++) {
                ...
                auto laserCloudCubeCornerPointerKd =
                        laserCloudCornerKdMap[ToIndex(i, j, k)];
                auto laserCloudCubeSurfPointerKd =
                        laserCloudSurfKdMap[ToIndex(i, j, k)];
                auto laserCloudCubeNonFeaturePointerKd =
                        laserCloudNonFeatureKdMap[ToIndex(i, j, k)];

                auto laserCloudCubeCornerPointer =
                        laserCloudCornerArray[ToIndex(i, j, k)];
                auto laserCloudCubeSurfPointer =
                        laserCloudSurfArray[ToIndex(i, j, k)];
                auto laserCloudCubeNonFeaturePointer =
...
size_t MAP_MANAGER::FindUsedCornerMap(const PointType& p, int cenWidth, int cenHeight, int cenDepth) {
    ...
    std::size_t cubeInd;
    if(!pntToCubeIdx(p, const int& cenDepth, const int& cenWidth, const int& cenHeight, cubeInd)) {
        cubeInd = 5000;
    }
    return cubeInd;
}
...
{
    return FindUsedCornerMap(p, a, b,c);
    // int cubeI = int((p->x + 25.0) / 50.0) + c;
    // int cubeJ = int((p->y + 25.0) / 50.0) + a;
    // int cubeK = int((p->z + 25.0) / 50.0) + b;
