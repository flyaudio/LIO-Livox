...
boost::shared_ptr<std::list<Estimator::LidarFrame>> lidarFrameList;//todo:g_slideWinFrame
pcl::PointCloud<PointType>::Ptr g_curCloud;
Estimator* estimator;

ros::Publisher g_pubLaserOdometry;
ros::Publisher g_pubLaserOdometryPath;
ros::Publisher g_pubFullLaserCloud;
tf::StampedTransform laserOdometryTrans;
tf::TransformBroadcaster* g_tfBroadcaster;
ros::Publisher g_pubGps;
...
void pubOdometry(const Eigen::Matrix4d& newPose, double& timefullCloud) {
    ...
    g_pubLaserOdometry.publish(laserOdometry);

    geometry_msgs::PoseStamped laserPose;
    laserPose.header = laserOdometry.header;
    laserPose.pose = laserOdometry.pose.pose;
    g_laserOdoPath.header.stamp = laserOdometry.header.stamp;
    g_laserOdoPath.poses.push_back(laserPose);
    g_laserOdoPath.header.frame_id = "/world";
    g_pubLaserOdometryPath.publish(g_laserOdoPath);
    ...
    g_tfBroadcaster->sendTransform(laserOdometryTrans);
...
void process() {
    ...
    while(ros::ok()) {
        newfullCloud = false;
        g_curCloud.reset(new pcl::PointCloud<PointType>());
        {
            ...
            if(!_lidarMsgQueue.empty()) {
                ...
                pcl::fromROSMsg(*_lidarMsgQueue.front(), *g_curCloud);
                _lidarMsgQueue.pop();
                newfullCloud = true;
            }
            // lock_lidar.unlock();
    ...
    // if(newfullCloud) {
        ...
        lidarFrame.laserCloud = g_curCloud;
        ...
        removeLidarDistortion(g_curCloud, delta_Rl, delta_tl);
        ...
        {// publish lidar points
            ...
            pcl::transformPointCloud(lidar_list->front().laserCloud, *tmp, transformTobeMapped.cast<float>());
            // int laserCloudFullResNum = lidar_list->front().laserCloud->points.size();
            // tmp->reserve(laserCloudFullResNum);
            // for (int i = 0; i < laserCloudFullResNum; i++) {
            //     PointType temp_point;
            //     pointAssociateToMap(&lidar_list->front().laserCloud->points[i], &temp_point, transformTobeMapped);
            //     tmp->push_back(temp_point);
            // }
            sensor_msgs::PointCloud2 out;
            pcl::toROSMsg(*tmp, out);
            out.header.frame_id = "/world";
            out.header.stamp.fromSec(lidar_list->front().timeStamp);
...
int main(int argc, char** argv)
{
    ...
    g_pubFullLaserCloud = nodeHandler.advertise<sensor_msgs::PointCloud2>("/livox_full_cloud_mapped", 10);
    g_pubLaserOdometry = nodeHandler.advertise<nav_msgs::Odometry> ("/livox_odometry_mapped", 5);
    g_pubLaserOdometryPath = nodeHandler.advertise<nav_msgs::Path> ("/livox_odometry_path_mapped", 5);
    g_pubGps = nodeHandler.advertise<sensor_msgs::NavSatFix>("/lidar", 1000);

    g_tfBroadcaster = new tf::TransformBroadcaster();
